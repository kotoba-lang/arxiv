...
% ---------------------------------------------------------------------------
\title{\textbf{\kotoba: A Content-Addressed Datalog Substrate\\
for Accountable Decentralized Agent Memory}}
...
\begin{abstract}
We present \kotoba, a content-addressed knowledge-graph database designed as a
durable, queryable, and accountable memory substrate for decentralized AI-agent
systems. \kotoba\ combines design primitives usually provided by separate
systems: (i) immutable, Datomic-style 5-tuple datoms whose transactions and
indices are named by IPFS-compatible content identifiers (\cid{}s); (ii) a
covering in-memory arrangement (EAVT/AEVT/AVET/VAET) mirrored by
history-independent Prolly Tree indices on disk; (iii) Datalog evaluation over
datom history, with a SPARQL~1.1 interoperability surface; (iv) Pregel
bulk-synchronous-parallel graph computation over datom-derived graphs; (v)
capability-based authorization via chain-agnostic CACAO (CAIP-74) delegation
chains; (vi) end-to-end encrypted agent messages using the Signal protocol
(X3DH + Double Ratchet); (vii) sandboxed polyglot user logic via the
WebAssembly Component Model, including a native safe Clojure-to-WASM compiler;
and (viii) a tiered, optionally AES-256-GCM-sealed block store backed by IPFS
export. We describe the data model, storage and indexing layers, query and
computation engines, security architecture, and runtime. We report micro- and
macro-benchmarks on commodity hardware (an Apple~M4 laptop), including a
sustained ingest rate of $5{,}222$ entities/second and CACAO-gated,
replay-protected query throughput of $12{,}753$ queries/second. We close with a
five-axis analysis of the open problems---authority, ordering, availability,
conflict, and transport---that shape the path from the current single-writer
per-graph deployment to a fully multi-writer decentralized one.
\end{abstract}
...
(b) reason over that graph with recursive, rule-based queries; (c) authenticate
and authorize one another using portable credentials rather than server-issued
sessions; (d) exchange private messages with end-to-end confidentiality; and
(e) execute untrusted, user-supplied logic safely. No single existing system
covers this surface: graph databases lack content-addressed provenance and
capability security; content-addressed stores lack a query engine; capability
frameworks lack a database; and agent frameworks lack durable, verifiable
persistence.
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  \item A \emph{content-addressed datom model} that gives facts, index nodes,
    transactions, and programs deterministic IPFS-compatible \cid{}s, making
    commit identity replica-independent
  ...
  \item A Datalog engine over immutable datom history, surfaced both natively
    and through SPARQL~1.1, and lifted to Pregel BSP computation
    (\cref{sec:datalog,sec:pregel}).
  ...
  \item A WebAssembly Component-Model runtime with a typed WIT host interface and
    a native Clojure-to-WASM compiler, enabling sandboxed polyglot user-defined
...
\kotoba\ is implemented in Rust as a workspace of over thirty crates. The
...
\texttt{kotoba-clj}     & Clojure $\rightarrow$ WebAssembly compiler. \\
...
Persistent blocks in \kotoba\ are named by a \cid: a self-describing
...
writes go to both tiers and reads fall through, promoting cold hits back to the
hot tier. On \texttt{wasm32}, \texttt{IdbBlockStore} backs the store with
browser IndexedDB.
...
The query engine evaluates Datalog programs over immutable datoms and records
derived facts as first-class results. For recursive programs, the natural
execution strategy is \emph{semi-naive} fixpoint iteration over change deltas:
rather than re-deriving all facts at each iteration, each rule is evaluated
against the newly derived delta of the previous round, and only genuinely new
facts propagate; iteration halts when a round produces nothing new. The
canonical example is transitive closure,
\[
...
A query result can be cached under a \cid\ derived from the triple (head-commit
\cid, graph \cid, query text). A repeat of the same query against the same head
is an $O(1)$ block fetch and decode; a new commit changes the key,
transparently invalidating the old view while keeping it addressable for
historical queries. The current implementation includes the materialized-view
primitive; wiring every recurring Datalog query into incremental maintenance is
ongoing work.
...
step is an invocation of the Datalog evaluator on the inbox. Each
...
A distributed runner can shard vertices across nodes and exchange inter-node
messages over the transport layer, sorting each inbox deterministically by source
...
among the granted resources. The non-signature portion of the check is small;
signature verification dominates. Under load the gate sustains the throughput
reported in \cref{sec:eval} with full replay protection. Every
...
queries, and LLM inference. Execution is metered by a gas model exposed by the
host, and each program is content-addressed by the \cid\ of its WASM
...
ships a native Clojure-to-WASM compiler (\texttt{kotoba-clj}) that parses EDN,
...
builds and the benchmark harness documented in the repository README. They are
intended to characterize orders of magnitude, not to establish competitive
rankings; absolute numbers depend on graph size, concurrency, and the cold-tier
backend (in-memory, LAN Kubo, WAN Kubo, or same-AZ S3).
...
AEVT reverse (B-tree range)  & $8\,\mu$s \\
AVET value lookup            & $18\,\mu$s \\
2-hop path                   & $748$\,ns \\
3-hop path                   & $1.16\,\mu$s \\
JOIN (AVET$\times$AVET, 10K entities)   & $783\,\mu$s \\
...
sandbox, into which a native Clojure~\cite{clojure} compiler emits. \kotoba's
contribution is not any one of these mechanisms but their coherent integration
behind a single content-addressed datom substrate.
...
\end{enumerate}
%
The storage-confidentiality roadmap is similarly staged: encryption at rest,
capability-gated reads, and access-receipt data structures exist in the current
system, while per-DID signed journals anchored to an L2 and custodian rotation
remain planned work.
...
programs content-addressed objects, the system gives agent memory stable,
verifiable identities, while a hot arrangement keeps interactive queries fast
and CACAO, Signal, and WASM provide portable security and safe extensibility.
Our evaluation indicates the design is practical on commodity hardware, and our
five-axis roadmap charts the remaining work toward full multi-writer
decentralization.
